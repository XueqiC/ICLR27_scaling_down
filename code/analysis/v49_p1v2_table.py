...
if rows:
    ...
    for r in rows:
        if r["regime"].startswith("prune"):
            L.append(f"{r['source'].replace('pythia-','').replace('@step','@')} & {r['protocol']} & {r['regime']} & {r['power']:.3f} & {r['A2']:.3f} & {r['A1']:.3f} & {r['cont']:.3f} & {r['strength_only']:.3f} & {r['zero']:.3f} & {r['best'].replace('_','-')} \\\\")
    L += [r"\midrule", r"Source & Prot. & Regime & full & no-D0 & mean & median & zero & & best \\", r"\midrule"]
    for r in rows:
        if r["regime"].startswith("quant"):
            L.append(f"{r['source'].replace('pythia-','').replace('@step','@')} & {r['protocol']} & {r['regime'].replace('>=', '$\\ge$')} & {r['full']:.3f} & {r['noD0']:.3f} & {r['per_bit_mean']:.3f} & {r['per_bit_median']:.3f} & {r['zero']:.3f} & & {r['best'].replace('_','-')} \\\\")
