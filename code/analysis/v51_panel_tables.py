...
PRUNE_DIR = {"Qwen3-8B": "Qwen--Qwen3-8B"}
def load_prune(m):
    p = R6 / PRUNE_DIR.get(m, m) / "prune_losses.json"
    if not p.exists(): return None
    j = json.load(open(p)); dense = j["1.0"]
    ...
    return dens, dl
def load_quant(m):
    p = R10 / QUANT_DIR.get(m, m) / "quant_losses.json"
    if not p.exists(): return None
    j = json.load(open(p)); dense = j["dense"]
...
L = [r"\begin{table}[t]\centering\footnotesize\setlength{\tabcolsep}{3.5pt}", r"\begin{tabular}{@{}lrrrrlcl@{}}", r"\toprule",
     ...
     r"\cmidrule(lr){3-5} & math & code & QA & & & & \\", r"\midrule"]
for r in rows:
    ...
    star = "" if r["cohort"] == "panel" else "$^{\\dagger}$"
...
L += [r"\bottomrule\end{tabular}",
      ...
      r"$^{\dagger}$prospective additions measured at four densities only. Generated by \texttt{v51\_panel\_tables.py} from \texttt{prune\_losses.json}.}",
      r"\label{tab:panel_prune}\end{table}"]
(TAB / "panel_prune.tex").write_text("\n".join(L) + "\n")
...
for r in rows:
    ...
    d4, d3 = r["dl4"], r["dl3"]; star = "" if r["cohort"] == "panel" else "$^{\\dagger}$"
...
L += [r"\bottomrule\end{tabular}",
      ...
      r"$^{\dagger}$prospective additions. Generated by \texttt{v51\_panel\_tables.py} from \texttt{quant\_losses.json}.}", r"\label{tab:panel_quant}\end{table}"]
(TAB / "panel_quant.tex").write_text("\n".join(L) + "\n")
# ---- counts
