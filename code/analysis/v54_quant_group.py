...
def _apply_grouped_quantization(
    ...
    if group_size is None or group_size == 0:
        _apply_fake_quantization(parameters, dense_weights, bits)
        return
    with torch.no_grad():
        for (_, parameter), dense in zip(parameters, dense_weights):
            parameter.copy_(fake_quantize_grouped(dense, bits, group_size))
...
def run_quantization(
    ...
    try:
        ...
        if "dense" not in results:
            ...
            results["dense"] = _measure_capability_losses(model, tokenizer, probes, device)
            _synchronize(device)
        ...
        for key, bits, group_size in pending:
            ...
            start = time.perf_counter()
            try:
                _apply_grouped_quantization(parameters, dense_weights, bits, group_size)
                losses = _measure_capability_losses(model, tokenizer, probes, device)
            finally:
