...
L = [r"\begin{table}[t]\centering\footnotesize", r"\begin{tabular}{@{}llrrrr@{}}", r"\toprule", r"capability & term & $\beta_{0}$ & $\beta_{\log N_0}$ & $\beta_{L_0}$ & $\beta_{\log D_0}$ \\", r"\midrule"]
for c in C:
...
L = [r"\begin{table}[t]\centering\footnotesize", r"\begin{tabular}{@{}llrrrr@{}}", r"\toprule", r"subset & candidate & math & code & QA & mean \\", r"\midrule"]
...
# ---- v55 LOSO table
lo = r5["loso_table"]
L = [r"\begin{table}[t]\centering\footnotesize", r"\begin{tabular}{@{}lrrr@{}}", r"\toprule", r"candidate & math & code & QA \\", r"\midrule"]
def getm(entry, c):
...
def to_tex(lines, caption, label, colspec):
    hdr = [c.strip() for c in lines[0].strip("|").split("|")]; out = [r"\begin{table}[t]\centering\scriptsize\setlength{\tabcolsep}{2pt}", r"\begin{tabular}{" + colspec + "}", r"\toprule", " & ".join(h.replace("_", r"\_") for h in hdr) + r" \\", r"\midrule"]
    for l in lines[2:]:
