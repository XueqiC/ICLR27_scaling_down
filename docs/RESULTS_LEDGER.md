...
| C3 | Block structure of benchmark gradient signatures | VERIFIED with stats on 1 model (block score 0.0995, boot CI [0.072,0.123], perm p=0.001) + point estimates on 7 | v9 reports + v9b stats.json | 7 | bootstrap+permutation (Qwen1.7B); extension queued | Fig. blocks |
...
| C18 | Distillation math/code loss rise is global/style drift, not capability loss (style-residualized dL_cap<0 in all 12 cells); QA genuine gain | PRELIMINARY (v16 control-task residual, c4 generic baseline; 2 students, single seed) | results/v16-style-residual/summary.md | 12 runs | generic-corpus control | exp §distill |
| C19 | Capability link A_c=g_c(L_c) is estimable and steep/monotone; math+code accuracy halves at loss ~1.0 and →0 by loss ~3, QA gradual (halves ~3.5); accuracy cliff coincides with loss cliff | PRELIMINARY (gemma3-12b easy benchmark GSM8K/MBPP/TriviaQA, 5-point prune ladder, 1 model) | results/v15-accuracy/gemma3-12b/*easy* | 1 model, 3 caps | link fit on held-out benchmark | prelim §link |
