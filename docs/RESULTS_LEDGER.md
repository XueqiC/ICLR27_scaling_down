...
### C77. Four-corner second difference, part 2: the 4B verdict and the registered decision (GPU + CPU, 2026-09-15; results/a5-corner-second-difference)

4B corners landed to the token as well: 50558 / 50563 (existing) / 105433 / 105450. Raw probe deltas and the statistic I = d3 - d4 - d1 + d2:
| readout | d1 | d2 | d3 | d4 | I(4B) | band 2*sigma_I | I(1B) | same sign |
| QA probe (PRIMARY) | -1.4196 | -1.0696 | -1.1527 | -0.5904 | -0.2123 | 0.3216 | -0.2434 | yes |
| MATH-500 (marginal) | 0.1266 | 0.1066 | 0.1543 | 0.1711 | -0.0369 | 0.0176 | +0.0076 | no |
| MBPP (UNDERPOWERED) | 0.1430 | 0.0978 | 0.1390 | 0.1885 | -0.0946 | 0.0508 | -0.0140 | yes |
REGISTERED DECISION, PRIMARY READOUT: both students INSIDE the band, same sign, each about 1.3-1.5 sigma from zero and six to seven times SMALLER than the 1.55 nats an interacting structure predicted at these corners. ADDITIVITY SURVIVES its one fit-free test on the readout that had the power to reject it. The failure of the three fitted structures in C74 is therefore attributed to the form of the individual terms (the reuse term is near-linear, not logarithmic, per A2's profile) or to irreducible variation, NOT to a missing budget-by-reuse interaction.
MATH (marginal): 4B outside the band, 1B inside with the opposite sign. Under the registered rule this is not a rejection (both students must be outside with the same sign) and is reported as a size-dependent difference, not averaged away.
CODE (UNDERPOWERED, registered as such before training): 4B I=-0.0946, 3.7x the registered noise and 6.6x the predicted disagreement, while 1B sits inside the band with the same sign. Not a rejection under the rule; reported exactly as registered: an underpowered readout on which the larger student shows a second difference the smaller one does not. This is the one place the corner data points at something the registration did not anticipate, and it is recorded as a lead for a future design that gives code real power, not as a finding of this round.
SECONDARY CHECK (fresh-sample distributions, all four corners of each student evaluated on GPU0, noise from only 2 seed pairs as registered): 4B 2wiki_new I=-0.3070 (band 0.7646; 1B -0.3339) inside, same sign; musique -0.0100 (band 1.672; 1B +0.0434) inside; triviaqa -0.6585 (band 0.288; 1B -0.1134) OUTSIDE on 4B, inside on 1B, same sign. The same pattern as math and code: the larger student shows a second difference the smaller does not, on a readout whose noise estimate is weak. Recorded as consistent with the primary on 2wiki_new and as a further size-dependent lead on triviaqa; not a rejection under the rule. A5 artifact: results/a5-corner-second-difference/summary.md, regenerated with both students.
WHAT THE PAPER CAN SAY: at fixed student, the distillation response is additive in training budget and data reuse to within the pool-seed noise on the primary readout, on two students spanning 4.6x in non-embedding size; the earlier falsification (C71) was of the logarithmic reuse term, not of additivity; and the larger student shows second differences on math and code that the smaller does not, at readouts that were marginal or underpowered by design.

### C78. Correction to the reading of C77, and the closeout checks (CPU, 2026-09-15)

CORRECTION. C77 and the first FINAL_RESEARCH_VERDICT said additivity "survives" and attributed the fitted structures' failure to the form of their individual terms. Both overstate the evidence. The supported statement is: in the budget-and-reuse region measured, on the two students measured, the primary QA check FAILED TO REJECT additivity; the frozen interaction candidate clearly overestimated the second difference. Failing to reject is not establishing; the measured values (-0.24 on 1B, -0.21 on 4B) lie inside the registered band but not inside any pre-defined equivalence margin, so no practical-additivity claim is made; an interaction is not excluded; and no inference about why the fitted structures failed follows. Math, code and TriviaQA deviations on the 4B remain scope statements and leads; the QA result is stated for QA only. The verdict document has been rewritten accordingly. C77's tables stand; its interpretive sentences are superseded by this entry.
CLOSEOUT CHECKS (results/a7-closeout-audit, paper/docs/CLOSEOUT_AUDIT.md): (1) the Stage A gate re-read against the development-stage (inner-selected) baseline for both primary targets, with the outer-oracle comparison demoted to diagnostic; (2) the 4B parameter accounting written out with explicit deductions -- 4,300,079,472 total, minus 671,252,480 tied embedding, minus 419,816,304 vision tower and projector, equals 3,209,010,688 text-decoder non-embedding; 3.629B is the figure with the vision deduction omitted; (3) the training-seed comparison re-presented as ONE pair of runs whose 14 checkpoint differences form a correlated trajectory, with the differences at the two corner checkpoints stated, and no claim that the registered band is generally conservative; (4) per-student frozen second-difference predictions of F_int at the ACHIEVED corners against the measured values, replacing the "six to seven times" summary. VERSION CONSISTENCY: the public mirror, generated tables and paper numbers move to the nine-state V92 panel, with the six-state snapshot kept as labelled history. Outcomes of these checks are recorded in the audit document when they complete.
DIRECTIVE RECORDED: the round closes here. The remaining ~41 GPU-hours are retained; reopening experiments requires a new information source or mechanism hypothesis as a new research stage, not another reuse function.

### C79. Closeout audit outcomes, including a correction to what the registration called the "predicted disagreement" (CPU, 2026-09-15; results/a7-closeout-audit, paper/docs/CLOSEOUT_AUDIT.md)

GATE (check 1): re-read against the DEVELOPMENT-STAGE (inner-selected) baseline, the launch verdict does not change for either primary target. code x I_T: 3/12 scorable folds have positive gain, 1/12 positive with an interval clear of zero, 0/4 splits clear. QA x I_U (fresh 2wiki_new, 1% proxy): 4/11 positive, 0/11 clear of zero, 0/3 splits clear. The outer-oracle comparison is retained only as a labelled diagnostic.
PARAMETERS (check 2): 4B = 4,300,079,472 total - 671,252,480 tied embedding - 419,816,304 vision tower and projector = 3,209,010,688 TEXT-DECODER NON-EMBEDDING (revision cc012e0a); 3.629B is the figure with the vision deduction omitted. 270M 100,326,016; 1B 697,896,064; 12B 10,759,155,456, same accounting.
SEED PAIR (check 3): re-presented as one pair of runs; its 14 checkpoint differences are correlated observations of one trajectory, not replicates; differences at the two corner checkpoints reported per readout; no claim about the band being generally conservative is made. C76's "about half the registered figure" is withdrawn as a general statement.
CORRECTION (check 4). The registration's "predicted additive-vs-interaction disagreement" (0.0144 / 0.0215 / 1.5546) was copied from A2's MAX-over-candidate-corners design column, not from the interaction structure's own second difference, and at design rather than achieved corners. Evaluating the FROZEN full-development F_int at the ACHIEVED corners (coefficients, standardizers and descriptors from A2's stored fit records, no refit): 2Wiki QA probe I_pred = -0.489 on both students (the k(u_hi-u_lo)(v_lo-v_hi) term dominates and is student-independent on a rectangle); MATH-500 -0.0032; MBPP -0.0034. Measured: QA -0.243 (1B) / -0.212 (4B); math +0.0076 / -0.0369; code -0.0140 / -0.0946. So on the primary readout the measured value sits between the additive prediction (0, error 0.24 / 0.21) and the frozen interaction prediction (-0.49, error 0.25 / 0.28), about 1.3-1.7 registered-noise units from EACH, with F_int's sign correct and its magnitude about twice the measurement. C77/C78's "clearly overestimated" is replaced by: the frozen interaction candidate overestimates the magnitude by about a factor of two with the correct sign, and neither structure predicts the measured value within the registered noise. The registered power ratio for the QA probe, recomputed against F_int's own prediction, is 0.489/0.161 = 3.0 (still powered); MATH-500 0.0032/0.0088 = 0.36 and MBPP 0.0034/0.0254 = 0.13, so MATH-500 is relabelled UNDERPOWERED against the frozen interaction candidate, not marginal. The 4B math and code measurements exceed BOTH structures' predictions by far; neither anticipated them.
STANDING VERDICT after this correction: on the primary readout, the pre-fixed rule did not reject additivity; the frozen interaction candidate was not rejected either by any pre-fixed rule, since none was registered for it; the measurement discriminates neither structure at this noise level. Everything else in C78 stands.

### C80. Paper restructured as one systematic study of predictive scaling-down laws (writing, 2026-09-15; Scaling_down_law_paper)

DIRECTIVE (user, 2026-09-15): present the whole paper as a complete systematic study; narrative per results section "observed -> conditions -> explanatory evidence -> predictive relation -> where it applies"; effect sizes, errors, uncertainty and range rather than success/failure; explanatory analysis prominent with verified causes / excluded explanations / unidentified factors; three contribution classes (responses and relations; scientific understanding; design and choice guidance) each tied to evidence; main table by method x prediction task; generalization figure separating new configurations from new sources, cross-distribution response shown separately; development, frozen and post-hoc statuses kept distinct; pre-registered verdicts preserved; positive impacts concrete and bounded.
WHAT CHANGED. Main text order: Introduction; Experimental Setup; What Compression Does to Capabilities (new, observations first); Why the Responses Vary: Conditions and Mechanism (new: function shape, source state/protocol/distribution, mechanism, and the three lists); Final Predictive Relations per Method (with the new main table); Independent Tests (with the new generalization figure); Selection (with the stated cross-family limit); Related Work; What Was Learned, and What It Enables (new, replaces the nine-line discussion). Abstract and introduction rewritten to the organising question and the three contribution classes. Moved to the appendix: per-arm selection and boundary paragraphs, conditioning on the initial state, applicability across capabilities, the old relations figure, the selection-map figure, the old main table and frozen-candidates figure (labels kept resolvable). Added to the appendix: the logit-displacement account with the three-family boundary table (C64/C70 evidence, previously absent from the paper) and pre-compression statistics as inputs (V92/A4, previously absent).
GENERATED ARTIFACTS (codex, numbers from frozen JSON with a sidecar mapping each numeric cell to its field): tables/main_prediction_v2.tex (method x task; relation and parameter count; tested range; MAE by capability; development-selected baseline and its MAE; status in {development, frozen prediction, post-hoc recommendation}; the 2.8B identical-weights pair is not presented as two states; "not tested" where no frozen artifact exists); figs/responses_v2 (student x reuse; three QA distributions); figs/explanation (curvature exponent across folds; corner test with additive and frozen-interaction predictions against the measurement and the registered band; displacement-account error against damage magnitude); figs/generalization (paired relation-versus-baseline MAE per row, whiskers only where a paired interval is stored; corner contrasts; an explicit statement that no frozen response-law prediction exists on fresh distributions) with the per-cell version in the appendix.
WORDING RULES APPLIED: "failed to reject" is the ceiling for the corner test; no practical-additivity claim; no attribution of the fits' failure to term form; the interaction candidate's own frozen prediction (-0.49) is quoted, not the design-table maximum; QA result stated for QA only; the cross-family selection validation stated as not completed.
