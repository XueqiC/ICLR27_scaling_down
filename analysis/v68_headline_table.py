...
and paper/paper/tables/pred_full.tex (all rows, label tab:pred_full, [!htbp]) for Appendix D. Numbers are copied verbatim."""
...
full = src.replace("\\label{tab:pred_main}", "\\label{tab:pred_full}").replace("\\begin{table}[!t]", "\\begin{table}[!htbp]", 1).replace("\\begin{table}[t]", "\\begin{table}[!htbp]", 1)
...
(T / "pred_full.tex").write_text(publication_table_text(full))
