    ...
    for tag in fz["test_states"]:
        ...
        for c in CAPS:
            ...
            for d in fz["test_densities"]:
                ...
                cell["pred"]["zero_change"] = 0.0   # the no-change baseline: is the panel's response non-trivial?
                cells.append(cell)
            entry[c] = {"cells": cells,
...
def render():
    ...
    for key, word in (("zero_change", "Zero change"), ("pythia_zero_calibration", "Pythia coefficients, no refit")):
        vals = [sc["summary"][cap][key][b] for b in ("probes", "new") for cap in CAPS]
        rows.append(" & ".join([word, "None"] + [f"{v:.3f}" for v in vals]) + r" \\")
    ...
    lines = ["# A18 per-state mean absolute error (nats per token), primary comparison and competitors", "",
             "| State | Capability | Items | " + " | ".join(f"{FORM_WORDS[f]} ({g})" for g in ("reduced", "full") for f in FORMS) + " | Pythia, no refit | Zero change |",
             "|---|---|---|" + "---:|" * (2 * len(FORMS) + 2)]
    for tag, e in sc["per_state"].items():
        for cap in CAPS:
            for b, word in (("mae_probes", "probes"), ("mae_new", "new")):
                lines.append(f"| {tag} | {cap} | {word} | " + " | ".join(f"{e[cap][b][f'{f}_{g}']:.3f}" for g in ("reduced", "full") for f in FORMS) + f" | {e[cap][b]['pythia_zero_calibration']:.3f} | {e[cap][b]['zero_change']:.3f} |")
...
def render_body():
    ...
    caption = ("Same-complexity forms at half the development budget: mean absolute error in nats per token on the four "
               ...
               "OLMo-2 test states with 12 configurations, every form frozen before the test. "
               "The per-density regression fits one regression per density, nine Pythia states at each of two densities.")
