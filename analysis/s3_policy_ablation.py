...
def render_body(summary):
    """Condensed body table: pooled regret of each policy per objective, beside the opportunity."""
    from analysis.paper_table_layout import house_style, table_layout
    rows = []
    for objective in ("math", "code", "qa"):
        e = summary["pooled"][objective]
        rows.append(" & ".join([OBJECTIVE_WORDS[objective], f"{e['mean_opportunity']:.3f}", f"{e['priority']:.3f}", f"{e['median']:.3f}",
                                f"{e['relations']:.3f}", f"{e['rule']:.3f}", f"{e['quant-only']:.3f}"]) + r" \\")
    caption = ("Mean regret of each selection policy on the four independent references, in nats per native token, pooled over "
               "references and the sixteen paired budgets, beside the cross-method opportunity over the best quantization "
               "candidate; policies as defined in the text, full table in Appendix Table~\\ref{tab:s3-ablation}.")
    text = ("% Generated by analysis/s3_policy_ablation.py; retrospective, do not edit.\n"
            "\\begin{table}[tb]\n\\centering\\footnotesize\n"
