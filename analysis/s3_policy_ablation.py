...
def render(summary):
    ...
    for i, rid in enumerate(order):
        for j, objective in enumerate(OBJECTIVES):
            ...
            cells = [NAMES[rid] if j == 0 else "", LABELS[objective], fmt(e["mean_opportunity"]),
                     *[fmt(e[n]["mean_regret"]) for n in ("priority", "median", "relations", "rule")],
    ...
    caption = (
        ...
        "of Section~\\ref{sec:laws} where their inputs exist and the selected distillation forms; the final rule is the "
        "policy fixed before the round, and its quantization-only counterpart is in Table~\\ref{tab:s3-selection}. "
        "The rule gain is the loss of the best quantization candidate minus that of the rule's choice, the gain the "
        "rule realises even against a perfect quantization selection. The three added policies were defined after "
        "the round from development data alone.")
...
BODY_TABLE = ROOT / "paper/paper/tables/s3_ablation_body.tex"
OBJECTIVE_WORDS = {"math": "Mathematics", "code": "Code", "qa": "Question answering", "multi": "Largest increase"}


FAMILY_WORDS = {"pythia": "Pythia 410M and 1.4B", "gemma": "Gemma 3 1B and 4B"}


def family_pools(summary):
    """Per-family means over the two references that share a tokenizer (native nats are never pooled across tokenizers)."""
    per = summary["per_reference"]
    out = {}
    for family in FAMILY_WORDS:
        out[family] = {}
        for objective in OBJECTIVES:
            cells = [v for k, v in per.items() if k.split("--")[0].startswith(family) and k.endswith("|" + objective)]
            if not cells:
                cells = [v for k, v in per.items() if family in k and k.endswith("|" + objective)]
            out[family][objective] = {"n_references": len(cells),
                                      "mean_opportunity": statistics.mean(c["mean_opportunity"] for c in cells),
                                      **{name: statistics.mean(c[name]["mean_regret"] for c in cells) for name in POLICIES}}
    return out


def render_body(summary):
    """Condensed body table: per family and objective, the regret of each policy beside the opportunity."""
    from analysis.paper_table_layout import house_style, table_layout
    pools = family_pools(summary)
    rows = []
    for i, family in enumerate(FAMILY_WORDS):
        for j, objective in enumerate(("math", "qa")):
            e = pools[family][objective]
            rows.append(" & ".join([FAMILY_WORDS[family] if j == 0 else "", OBJECTIVE_WORDS[objective], f"{e['mean_opportunity']:.3f}",
                                    f"{e['priority']:.3f}", f"{e['median']:.3f}", f"{e['relations']:.3f}", f"{e['rule']:.3f}",
                                    f"{e['quant-only']:.3f}"]) + r" \\")
        if i == 0:
            rows.append(r"\midrule")
    caption = ("Mean regret of each selection policy in nats per native token per family, native nats never pooled across "
               "tokenizers, over the sixteen paired budgets, beside the cross-method opportunity over the best quantization "
               "candidate; the code rows, where the opportunity is below 0.01 nats and every policy ties, and the full table are "
               "given in Appendix Table~\\ref{tab:s3-ablation} with the same definitions.")
    text = ("% Generated by analysis/s3_policy_ablation.py; retrospective, do not edit.\n"
