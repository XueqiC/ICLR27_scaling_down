...
rows = [
 ("Pruning", r"$A_c(\mathbf x)\,((1-d)/0.3)^{\gamma_c}$ (Eq.~\ref{eq:power}); same-input alternative A2 (per-$d$ regression + interpolation)", "5 (A2: 20)", r"$d$; 17 Pythia states (160M--6.9B, 16k--143k)", r"$d\in[0.55,0.9]$ interp.\ on new states; $d=0.55$ extrap.\ fails", r"Table~\ref{tab:round3_coef}"),
 ("Quantization, fixed $b$", r"per-bit regression $\beta_{c,b}\cdot\phi(\mathbf x)$", "4 per $b$", r"new state at seen $b\in\{8,6,4,3\}$; 9 dev states", r"seen $b$; 5-bit rule fails; near-zero regime uninformative", "register v38/v49"),
 ("Quantization, $(b,g)$", r"$\phi(\mathbf x)\cdot[1,u,v,uv,u^2]$ (Eq.~\ref{eq:quant2d})", "20", r"$b\in\{3,5\}\times g\in\{64,256\}$; 6 dev states", r"unseen $b=4$, $g=128$ on dev states; new state mixed", r"Table~\ref{tab:quant2d_coef}"),
 ("Distillation, source axis", r"$\delta_c=\beta_c\cdot\phi(\mathbf x)$ (initial student)", "4", "9 Pythia students, fixed pool/budget", "held-out stage; math only", "register v39"),
 ("Distillation, pool axis", r"$a_c+b_c\log E$ (and constant, $\log T$, joint)", "2", "Gemma-3-1B; pools U75/U600", "unseen pool U225: QA only; multi-student test frozen", "register v41/v50"),
]
L = [r"\begin{table}[t]\centering\footnotesize\setlength{\tabcolsep}{3pt}", r"\begin{tabular}{@{}p{1.9cm}p{3.8cm}p{0.9cm}p{2.7cm}p{2.4cm}p{1.3cm}@{}}", r"\toprule",
     r"Arm & Predictor & $P$/cap. & Varied setting; dev.\ states & Tested range & Coeff. \\", r"\midrule"]
for r in rows: L.append(" & ".join(r) + r" \\")
L += [r"\bottomrule\end{tabular}", r"\caption{Predictive models and their domains. Inputs are always $\mathbf x=(N_0,D_0,L_{0,c})$ through $\phi(\mathbf x)=[1,\tilde{\log N_0},\tilde L_{0,c},\tilde{\log D_0}]$; no prediction uses a measurement of the compressed target. Ranges summarize \S\ref{sec:general}; coefficients and standardization constants are in Appendix~\ref{sec:app_coef}.}", r"\label{tab:models}\end{table}"]
...
(TAB / "v56_forms.tex").write_text(publication_table_text(tex))
