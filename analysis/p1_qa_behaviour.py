...
LONG = OUT / "summary_long.json"


def render_table(summary):
    """Appendix table: per reference and model, fresh-item loss, exact match under both readouts, and the paired loss."""
    from analysis.paper_table_layout import house_style, table_layout
    fmt = lambda x: f"{x:.3f}"
    long = read(LONG)["references"] if LONG.exists() else None
    def ci(d, m):
    ...
    for i, ref in enumerate(refs):
        ...
        for j, cid in enumerate(order):
            ...
            paired = "0.00 (reference)" if "quant-only" in m["roles"] else ci(m.get("vs_quant_only"), "loss")
            if long:
                lm = long[ref]["models"][cid]
                rows.append(" & ".join([NAMES[ref] if j == 0 else "", name + (f" ({roles})" if roles else ""), fmt(m["mean_loss"]),
                                        fmt(m["exact_match"]), fmt(lm["exact_match_96_span"]), fmt(lm["token_f1_96_span"]),
                                        f"{100 * lm['truncated_rate_96']:.0f}", paired]) + r" \\")
            else:
                rows.append(" & ".join([NAMES[ref] if j == 0 else "", name + (f" ({roles})" if roles else ""), fmt(m["mean_loss"]),
                                        fmt(m["exact_match"]), fmt(m["token_f1"]), paired]) + r" \\")
        if i < len(refs) - 1:
    ...
    if long:
        ...
        caption = (
            ...
            "token F1 with 32 new tokens, and the paired per-item loss difference from the quantization-only choice "
            "with its 95\\% bootstrap interval over items; negative favours the row. Parentheses name the policies "
            "that chose the model.")
    text = ("% Generated by analysis/p1_qa_behaviour.py --table; retrospective, do not edit.\n" +
            ("\\begin{table}[tb]\n\\centering\\scriptsize\n" if long else "\\begin{table}[tb]\n\\centering\\footnotesize\n") +
            f"\\caption{{{caption}}}\n\\label{{tab:p1-behaviour}}\n" +
            ("\\begin{tabular*}{\\textwidth}{@{\\extracolsep{\\fill}}llrrrrrr@{}}\n\\toprule\n"
             "Reference & Model (policies) & Loss & Exact match, 32 tokens & Exact match, 96 tokens & Token F1, 96 tokens & "
             "Truncated (percent) & Loss difference from the quantization-only choice \\\\\n\\midrule\n" if long else
