...
\citep{ho2020constructing}. Each probe set fixes one evaluation
distribution $j$ for its capability, so the measured quantity is
$L_{c,j}$, the conditional loss of capability $c$ on distribution $j$;
we write $L_c$ with the fixed distribution implied and test in
Appendix~\ref{sec:app_measure} how far a response carries to other
distributions of the same capability. The observable is the
...
model of that arm; the teacher never plays the role of the source. The observable is the
...
prediction keeps its provenance label whatever the outcome. The reference anchor $L_{0,c}$ is the only measured input. Error is the mean
