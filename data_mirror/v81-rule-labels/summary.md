...
- `analysis/v80_rule_addenda.py` is updated in place and mirrored byte-for-byte to `paper/analysis/v80_rule_addenda.py`. Reproduce with `python -B analysis/v80_rule_addenda.py`. PDF/PNG previews and both table files are also saved under `results/v81-rule-labels/`; SHA256 provenance is in `manifest.json`.
