...
(Appendix~\ref{sec:app_forms}): the second form has the lowest error
of all forms on the 270M unseen-pool trajectories (0.022, 0.020, and
0.32 nats), the first fails for QA wherever it is tested because it
has no budget term, and neither improves on the frozen constant for
math and code on the 4B student. No joint student-pool-budget law is
claimed.
...
better than the frozen constant and than zero change, while on the
held-out 4B student it beats the constant at the development pools and
loses to it at the unseen pool (Table~\ref{tab:p2v2_test},
\S\ref{sec:unseen_states}). The delivered distillation law is thus the
joint form in $T$ and $E$ with its source term, valid inside the
development size range; beyond it the per-capability constant is the
safer predictor at a new pool.
