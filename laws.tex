...
\begin{figure}[!t]
\centering
\includegraphics[width=\textwidth]{figs/input_form_compact.pdf}
...
densities $0.65$ and $0.55$, Eq.~\eqref{eq:power} has MAE 0.371, 0.524,
and 0.759 against 1.37, 1.64, and 1.61 for the strength-only curve, so
reading the source state is what matters; A2 is better on math (0.283)
and code (0.279) and within noise on QA (0.818), so the power structure
adds nothing over the per-density regression on this panel. After the
