...
held-out 4B student every structured form loses to the frozen constant
(Table~\ref{tab:p2v2_test}, \S\ref{sec:unseen_states}). The delivered
distillation law is thus the joint form in $T$ and $E$ with its source
term, valid inside the development size range; beyond it only the
per-capability constant transfers.
